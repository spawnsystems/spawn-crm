...
function AlertIcon({ type }: { type: string }) {
  if (type === "destructive") return <AlertTriangle className="h-3.5 w-3.5 text-destructive shrink-0 mt-0.5" />;
  if (type === "warning") return <AlertTriangle className="h-3.5 w-3.5 text-warning-foreground shrink-0 mt-0.5" />;
            ...
            <Popover open={notifOpen} onOpenChange={setNotifOpen}>
              ...
              <PopoverContent align="end" className="w-80 p-0">
                ...
                <div className="divide-y divide-border">
                  {alerts.map((a, i) => (
                    <div key={i} className="flex items-start gap-3 px-4 py-3 hover:bg-muted/40 transition-colors cursor-pointer">
